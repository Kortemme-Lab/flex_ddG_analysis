...
talaris_table = readtable('../data/by_step/zemu_1.2-60000_rscript_simplified-t14-id_50-35000-partial.csv');
ref_table = readtable('../data/by_step/zemu_1.2-60000_rscript_validated-ref-id_50-35000-partial.csv');
...
fig.PaperUnits = 'inches';
fig.PaperPosition = [0 0 7 6];
print('zemu-sigmoid2-tal-feats.png','-dpng','-r300');

plotsample(Rtal);
print('zemu-sigmoid2-tal_posterior.png','-dpng','-r600');
...
fig.PaperUnits = 'inches';
fig.PaperPosition = [0 0 7 6];
print('zemu-sigmoid2-con-feats.png','-dpng','-r300');

plotsample(Rcon);
print('zemu-sigmoid2-con-posterior.png','-dpng','-r300');
...
plot( exp_data, Rcon.fhat, 'r.', 'color', cell2mat( current_palette(2)));
hold off;
title('(a) - GAM No backrub control');
xlim([-6 11]); ylim([-6 11]);
hold on;
...
fig = gcf;
fig.PaperUnits = 'inches';
fig.PaperPosition = [0 0 4.0 8.0];
print('zemu-sigmoid2-corrs-main.png','-dpng','-r600');

subplot(121);
plot(exp_data, ref_table.total, 'r.', 'color', cell2mat( current_palette(5)));
title('(a) - Flex ddG (REF energy)');
xlim([-6 11]); ylim([-6 11]);
hold on;
...
fig = gcf;
fig.PaperUnits = 'inches';
fig.PaperPosition = [0 0 8.0 4.0];
print('zemu-sigmoid2-corrs-supp.png','-dpng','-r600');
